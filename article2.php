				...
				<div class="container">
					<div class="col-md-12">
						<div class="row2">
							...
							<?php
								$full = $_GET['id'];
								$art = mysqli_query($db, "SELECT * FROM `serial` WHERE id= " . $full);
							...
							<?php		
									...
									mysqli_query($db, "UPDATE `serial` SET views = views + 1  WHERE id= " . $full);
							?>	
							...
							<div class="description_art">							
								...
								?>								
									<p><b>Категория:   &nbsp</b><a href="My_index2.php?id=<?php echo $art_cat['id'] ?>"><?php echo $art_cat['title']?></a></p>
								</div>
								<div class="years_art">
								...
								<div class="actor_art">
									<p><b>В ролях:  </b><font color = #00008B><?php echo $art_1['customer'] ?></font></p>
								</div>
								<div class="text_art">
							...
							<div class="rectangle6">
								...
								<?php
								$new = mysqli_query($db, "SELECT * FROM `new_serial`");
									while( $new_1 = mysqli_fetch_assoc($new)){
								?>
										<div  class="new_image_1"> 
											 <a href="new_article2.php?id=<?php echo $new_1['id'] ?>" title=<?php echo $new_1['title'] ?> ><div  class="new_image" style="background-image: url(images/<?php echo $new_1['image'] ?>);"></div></a>
										</div>											
								<?php 
									} 
								?>
...
<div class="block">
	...
	<h3>Комментарии</h3>
		...
		<?php	
			$idn = (int)$art_1['id'];
			$comments = mysqli_query($db, "SELECT * FROM `comment2` WHERE `articles_id` = '$idn' ORDER BY `id` DESC LIMIT 10");
...
<div id="comment-add-form" class="block1">	
	<h3>Добавить комментарий</h3>
		<div class="block_content">
			<form class="form" method="POST" action="article2.php?id=<?php echo $art_1['id']; ?>#comment-add-form">
				<?php
					if(isset($_POST['do_post'])){
						$errors = array();						
							...
							if(empty($errors)){
								...
								$textarea = $_POST['text'];
								$art_id = $art_1['id'];
									mysqli_query($db, "INSERT INTO `comment2`(
