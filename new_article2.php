					...
					<div class="col-md-12">
						<div class="row">
							<div class="rectangle_art">
								...
								<?php
									$full_new = $_GET['id'];
									$new_art = mysqli_query($db, "SELECT * FROM `new_serial` WHERE id= " . $full_new);
								...
								<?php		
										...
										mysqli_query($db, "UPDATE `new_serial` SET views = views + 1  WHERE id= " . $full_new);
								?>	
								...
								<div class="description_art">							
									...
									<div class="director_art">
										<p><b>Режиссер:  </b><font color = #00008B><?php echo $new_art_1['director'] ?></font></p>
									</div>
									<div class="actor_art">
										<p><b>В ролях:  </b><font color = #00008B><?php echo $new_art_1['customers'] ?></font></p>
									</div>
									<div class="text_art">
							...
							<div class="rectangle6">
								...
								<?php
								$new = mysqli_query($db, "SELECT * FROM `serial`");
									while( $new_1 = mysqli_fetch_assoc($new)){
								?>
										<div  class="new_image_1"> 
											 <a href="article2.php?id=<?php echo $new_1['id'] ?>" title=<?php echo $new_1['title2'] ?> ><div  class="new_image" style="background-image: url(images/<?php echo $new_1['image'] ?>);"></div></a>
										</div>											
								<?php 
									} 
								?>
...
<div class="block">
	...
	<h3>Комментарии</h3>
		...
		<?php	
			...
			$comments = mysqli_query($db, "SELECT * FROM `new_comment2` WHERE `articles_id` = '$idn' ORDER BY `id` DESC LIMIT 10");
...
<div id="comment-add-form" class="block1">	
	<h3>Добавить комментарий</h3>
		<div class="block_content">
			<form class="form" method="POST" action="new_article2.php?id=<?php echo $new_art_1['id']; ?>#comment-add-form">
				<?php
					if(isset($_POST['do_post'])){
						$errors = array();						
							...
							if(empty($errors)){
								...
								$textarea = $_POST['text'];
								$art_id = $new_art_1['id'];
									mysqli_query($db, "INSERT INTO `new_comment2`(
									...
									echo '<span style="color :green;font-weight: bold; margin-bottom: 10px; margin-left: 130px; display: block;">Комментарий успешно добавлен!!!</span>';
									 header("Location: new_article2.php");
							}else{
								//Вывести ошибку
