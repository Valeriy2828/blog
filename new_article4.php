					...
					<div class="col-md-12">
						<div class="row2">
							<div class="rectangle_art">
								...
								<?php
									$full_new = $_GET['id'];
									$new_art = mysqli_query($db, "SELECT * FROM `new_books` WHERE id= " . $full_new);
								...
								<?php		
										...
										mysqli_query($db, "UPDATE `new_books` SET views = views + 1  WHERE id= " . $full_new);
								?>	
								...
								<div class="description_art">							
									...
									<div class="actor_art">
										<p><b>Автор:  </b><font color = #00008B><?php echo $new_art_1['author'] ?></font></p>
									</div>
									<div class="text_art">
							...
							<div class="rectangle6">
								...
								<?php
								$new = mysqli_query($db, "SELECT * FROM `books`");
									while( $new_1 = mysqli_fetch_assoc($new)){
								?>
										<div  class="new_image_1"> 
											 <a href="article4.php?id=<?php echo $new_1['id'] ?>" title=<?php echo $new_1['title2'] ?> ><div  class="new_image" style="background-image: url(images/<?php echo $new_1['image'] ?>);"></div></a>
										</div>											
								<?php 
									} 
								?>
...
<div class="block">
	...
	<h3>Комментарии</h3>
		...
		<?php	
			...
			$comments = mysqli_query($db, "SELECT * FROM `new_comment4` WHERE `articles_id` = '$idn' ORDER BY `id` DESC LIMIT 10");
...
<div id="comment-add-form" class="block1">	
	<h3>Добавить комментарий</h3>
		<div class="block_content">
			<form class="form" method="POST" action="new_article4.php?id=<?php echo $new_art_1['id']; ?> #comment-add-form">
				<?php
					if(isset($_POST['do_post'])){
						$errors = array();						
							...
							if(empty($errors)){
								...
								$textarea = $_POST['text'];
								$art_id = $new_art_1['id'];
									mysqli_query($db, "INSERT INTO `new_comment4`(
									...
									echo '<span style="color :green;font-weight: bold; margin-bottom: 10px; margin-left: 130px; display: block;">Комментарий успешно добавлен!!!</span>';
									 header("Location: new_article4.php");
							}else{
								//Вывести ошибку
